...
namespace StrideFlyTracking
{
    class GeoCalc
    {
        ...
        public static double DecimalMinutesToDecimalDegrees(string DMm)
        {
            double dotd;
            double decDegrees;

            int degrDigits = ((DMm.Length == 9) ? 2 : 3);  // Latitude or Longitude?

            dotd = Convert.ToDouble(DMm.Substring(degrDigits)) / 60;    
            decDegrees = Convert.ToDouble(DMm.Substring(0,degrDigits)) + dotd;

            return decDegrees;
        }

        public static double LetterToSignedDesignation(double degrees, string letter)
        {
            double result = degrees * ((letter.Equals("W") || letter.Equals("S")) ? -1 : 1);
            return result;
        }
    }
}
